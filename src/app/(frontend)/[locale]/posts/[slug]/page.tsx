...
export default async function Post({ params: paramsPromise }: Args) {
  ...
  const imageUrl =
    typeof post.heroImage === 'object' && post.heroImage?.url ? (post.heroImage.url.startsWith('http') ? post.heroImage.url : `${serverUrl}${post.heroImage.url}`) : null

  const breadcrumbJsonLd = getBreadcrumbJsonLd([
    { name: locale === 'fr' ? 'Accueil' : 'Home', url: `${serverUrl}/${locale}` },
    { name: locale === 'fr' ? 'Actualités' : 'News', url: `${serverUrl}/${locale}/posts` },
    { name: post.title, url: postUrl },
  ])

  // getProjectJsonLd outputs an 'Article' schema which is appropriate for blog posts
